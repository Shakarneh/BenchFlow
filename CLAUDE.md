...
merge — that is the CD half of CI/CD. Production readiness: **gunicorn** (not runserver),
**WhiteNoise** for static files, `DATABASE_URL` support via `dj-database-url`, `/api/health/`
health check, and `RENDER_EXTERNAL_HOSTNAME` trusted for `ALLOWED_HOSTS`.

⚠️ **Two live-deployment gotchas, both hit and fixed:** the assigned hostname had a random suffix
(`benchflow-qfzq`, not `benchflow`) so `ALLOWED_HOSTS` rejected everything; and `SECURE_SSL_REDIRECT`
turned the platform's internal http health check into a 301, so Render concluded the app was dead —
fixed with `SECURE_REDIRECT_EXEMPT`. Free tier has **no Shell**, so the superuser and demo data are
created at container startup from `DJANGO_SUPERUSER_*` env vars (password stored in Render, never
in git). Free instances **sleep after 15 min** — first visit takes ~50s.

**Earlier (still true):** `docker compose up` starts the **whole system** — web +
...
`demo1234`. Login pages for the browsable API at `/api/auth/`. Swagger at `/api/docs/`.
**JWT deliberately skipped** — session auth is enough for one Django app; JWT earns its keep when
a separate frontend or mobile client appears (would slot into DRF auth classes without touching
views). Phase 11 work is on `feat/phase-11-auth`.
...
he was copy-pasting without understanding. New rules: max ~3 plain sentences before any command;
one concept per step, concrete example before jargon; one plain sentence above every code block;
keep the after-green verification questions (he values those); deep dives deferred to the Grand
Walkthrough. Goal at this pace = the map, not every brick.
